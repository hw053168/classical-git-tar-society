...
const SubmissionCard = ({ submission, onVote, loading }: CardProps) => {
  const { publicKey, account } = submission;
  // Use the camelCase `youtubeId` from the type
  ...
  return (
    <div className="submission-card">
      ...
      <div className="submission-info">
        ...
        <p>
          {/* Use .toString() for the BN object */}
          Total Votes: <strong>{account.voteCount.toString()}</strong>
        </p>
        <button onClick={() => onVote(publicKey)} disabled={loading}>
          {loading ? "..." : "Vote for this Submission"}
        </button>
      </div>
    </div>
  );
